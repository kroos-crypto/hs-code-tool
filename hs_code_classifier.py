...
def classify_hs_code(row):
    ...
    # =========================================================================
    if cat == 'boots':
        return _classify_boots(material, primary_mat)

    if cat == 'rubberboots' or cat == 'rubber boots':
        return '6402991000'

    if cat == 'summer shoes':
        return _classify_summer_shoes(name, material, primary_mat)

    if cat == 'heeled shoes':
        # Heeled shoes with leather upper → 6403599100 (not covering ankle, leather sole class)
        if primary_mat == 'rubber':
            return '6402991000'
        return '6403599100'

    if cat == 'sneakers':
        return _classify_sneakers(material, primary_mat)
    ...
    if cat == 'winter shoes':
        # Winter shoes = usually ankle-covering leather boots
        if primary_mat == 'rubber':
            return '6402991000'
        if primary_mat == 'leather':
            return '6403511100'
...
def _classify_boots(material, primary_mat):
    """Classify boots category."""
    m = material.lower() if material and str(material) != 'nan' else ''
    # Rubber boots
    if primary_mat == 'rubber':
        return '6402991000'
    # Mixed leather + rubber sole (like 60% Calf + Polyester + PU upper, rubber sole)
    ...
    if primary_mat == 'leather':
        if has_leather_sole(material):
            return '6403511100'
        # Leather upper + rubber sole (common)
        return '6403511190'
    # Synthetic/textile upper
    if primary_mat == 'synthetic':
        return '6402991000'
    # Default: leather ankle boots
    return '6403511100'


def _classify_summer_shoes(name, material, primary_mat):
    """Classify summer shoes: ballerinas, sandals, flats, heeled sandals."""
    n = name.lower() if name and str(name) != 'nan' else ''
    m = material.lower() if material and str(material) != 'nan' else ''

    # Textile/synthetic upper (e.g. platform wedge sandals with polyamide upper)
    ...
    if primary_mat == 'leather':
        ...
        if is_sandal:
            if leather_sole:
                ...
                return '6403999190'   # Sandals, leather upper + rubber sole
        else:
            # Ballerinas, flats, loafers, pumps, slingbacks (not sandals)
            return '6403599100'       # Not covering ankle, leather sole/upper

    # Unknown material → assume leather construction (common for luxury brands)
    is_sandal = is_sandal_or_thong(name, material)
    if is_sandal:
        return '6403200000'
    return '6403599100'
...
def _classify_sneakers(material, primary_mat):
    """Classify sneakers / sports shoes."""
    m = material.lower() if material and str(material) != 'nan' else ''

    # If ANY leather is in the upper, use leather sneaker code
    ...
    if primary_mat in ('cotton', 'synthetic', 'artificial', 'linen'):
        return '6404199000'
    # Check explicitly for textile upper
    if 'upper' in m:
        if any(x in m.split('upper')[1][:80] for x in ['cotton', 'polyamide', 'polyester',
                                                          'nylon', 'canvas', 'mesh']):
            return '6404199000'
    # Unknown → leather sneaker (common for luxury/fashion brands)
    return '6403999890'
